...
function setup(props) {
    render(_jsx("div", { role: "main", children: _jsx(TimeAgo, { ...props }) }));
...
describe("TimeAgo", () => {
    ...
    `("renders $date as $output in $locale", async ({ date, locale, output }) => {
    ...
    `("renders $date as $output", ({ date, output }) => {
      ...
      ${"2023-06-06T10:59:00Z"} | ${"1 minute ago"}
      ${"2023-06-06T10:58:59Z"} | ${"1 minute ago"}
    `("renders $date as $output if `allowFuture` is true", ({ date, output }) => {
            const div = setup({
                date,
                hideSeconds: true,
                hideSecondsText: ["just now", "right now"],
    ...
    `("renders future $date as $output using `roundStrategy` floor", ({ date, output }) => {
    ...
    `("renders $date as $output using `roundStrategy` $roundStrategy", async ({ date, roundStrategy, output }) => {
      ...
      ${"2024"}       | ${"hace 1 minuto"}
    `("doesn't crash if someone tries to use ceil with $date", ({ date, output }) => {
    ...
    `("renders a tooltip for $date in $locale", ({ date, locale, tooltip, isoDate }) => {
            const element = setup({
                date,
                hideSeconds: true,
                locale,
...
describe("TimeAgoProvider", () => {
    function setupWithProvider(props, providerProps) {
        render(_jsx(TimeAgoProvider, { ...providerProps, children: _jsx("div", { role: "main", children: _jsx(TimeAgo, { ...props }) }) }));
        return screen.getByRole("main");
    }
    it("uses options from the context provider", () => {
        expect(setupWithProvider({ date: "2023" }, { locale: "ru-Cyrl-RU" })).toHaveTextContent("5 месяцев назад");
    });
    it("prefers props over values from the context provider", () => {
        expect(setupWithProvider({ date: "2023", locale: "it-CH" }, { locale: "ru-Cyrl-RU" })).toHaveTextContent("5 mesi fa");
    });
});
